...
export function PlayerBar({ narration, voiceId, onSelectVoice, onOpenDrawer }: Props) {
  ...
  return (
    /* What holds the bar on the reader — and what its own layout is measured
       against, since it is exactly the bar's width; see the stylesheet. */
    <div className={styles.host}>
      {/* The voice menu opens out of the bar and measures itself against it. */}
      <div
        className={`island ${styles.bar}`}
      ...
      >
        ...
        {/* A reading that never loaded leaves every control dead and, until
            this existed, said nothing at all about why. The message takes the
        ...
        {/* The two controls the row runs out of room for, in one box so that
            they can scroll together instead of wrapping to a line of their own.
            Above the breakpoint the box is not there at all (`display:
            contents`) and the bar lays them out itself — which is why they are
            in the DOM where they are read. */}
